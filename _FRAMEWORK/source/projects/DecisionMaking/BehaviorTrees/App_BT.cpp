...
App_BT::~App_BT()
{
	...
	SAFE_DELETE(m_pGuardBoard);
	for (auto& behavior : m_PatrolSelector)
		SAFE_DELETE(behavior);
	for (auto& behavior : m_PatrollPartialSequence)
		SAFE_DELETE(behavior);
	for (auto& behavior : m_ChaseSequence)
		SAFE_DELETE(behavior);
}
...
void App_BT::Start()
{
	...
	//1. Create Blackboard
	m_pGuardBoard = CreateBlackboard(m_pGuard);
	m_PatrollPartialSequence.push_back(new BehaviorAction(BT_Actions::StartPatrol));
	m_PatrollPartialSequence.push_back(new BehaviorAction(BT_Actions::Patrol));
	m_ChaseSequence.push_back(new BehaviorConditional(BT_Conditions::IsTargetVisible));
	m_ChaseSequence.push_back(new BehaviorAction(BT_Actions::Chase));
	m_PatrolSelector.push_back(new BehaviorSequence(m_ChaseSequence));
	m_PatrolSelector.push_back(new BehaviorPartialSequence(m_PatrollPartialSequence));
	//m_pPatrolSelector = new BehaviorSelector()
	...
	m_pBehaviorTree = new BehaviorTree(m_pGuardBoard, new BehaviorSelector(m_PatrolSelector));
	
	//3. Set the BehaviorTree active on the agent 
	m_pGuard->SetDecisionMaking(m_pBehaviorTree);
}
...
Elite::Blackboard* App_BT::CreateBlackboard(SmartAgent* smartAgent)
{
	...
	//pBlackboard->AddData("IsPatrolling",&m_IsPatrolling)
	// BT TODO:
	//...
